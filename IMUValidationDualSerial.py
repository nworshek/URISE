...
class DualSerialIMUValidationApp:
    ...
    def validate_test_inputs(self, require_motor=True):
        ...
        try:
            ...
            messagebox.showerror("Error", "Intensity, duration, cycles, and intensity step must be integers")
            return None
    ...
    def start_next_cycle(self, send_first=False):
        ...
        command = f"TEST:START:{motor_string}:{self.current_cycle_intensity}:{duration_ms}"
        ...
        self.realtime_summary_label.config(
            text=f"Live summary: cycle {self.current_cycle}/{self.cycle_count} running | intensity {self.current_cycle_intensity}"
        )
    ...
    def get_raw_fieldnames(self):
        ...
        # Include imu_timestamp only if any row actually has it.
        if any("imu_timestamp" in row for row in self.log_rows):
            base_fields.append("imu_timestamp")

        return base_fields + self.get_imu_column_names()
